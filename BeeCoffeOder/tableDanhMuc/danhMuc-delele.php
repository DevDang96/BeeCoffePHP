...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['Id_danhMuc'])) {
        ...
        $sql_check_product = "SELECT * FROM `danhmuc` WHERE `Id_danhMuc` = :Id_danhMuc";
        $stmt_check_product = $objConn->prepare($sql_check_product);
        $stmt_check_product->bindParam(':Id_danhMuc', $Id_danhMuc);
        ...
        try {
            $sql_delete = "DELETE FROM `danhmuc` WHERE `Id_danhMuc` = :Id_danhMuc";
            $stmt_delete = $objConn->prepare($sql_delete);
            $stmt_delete->bindParam(':Id_danhMuc', $Id_danhMuc);
...
<body>
    ...
    <form action="danhMuc-delete.php" method="POST">
        <label>id sản phẩm: <input type="text" name="Id_danhMuc"></label><br>
